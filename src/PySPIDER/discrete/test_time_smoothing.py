...
def create_test_datasets(positions, vs, deltat, dims, time_sigma_values=[0, 2.0]):
    ...
    observables = [
        ...
        Observable(string="v", rank=1, indices=(LiteralIndex(0),)),  # x-velocity component
        Observable(string="v", rank=1, indices=(LiteralIndex(1),)),  # y-velocity component
    ]
...
def create_primes():
    ...
    prime_rho = LibraryPrime(derivative=DerivativeOrder.blank_derivative(0, 0), derivand=cgp_rho)
    prime_rho_vx = LibraryPrime(derivative=DerivativeOrder.blank_derivative(0, 0), derivand=cgp_rho_vx)
    
    return prime_rho, prime_rho_vx


def compute_coarse_grained_fields(datasets, domain_size=(20, 20, 50), domain_offset=(10, 10, 25)):
    ...
    for time_sigma, dataset in datasets.items():
        ...
        field_rho_vx = dataset.eval_prime(prime_rho_vx, domain, experimental=True, order=4)
        ...
        print(f"rho[v_x] range: [{np.min(field_rho_vx):.3f}, {np.max(field_rho_vx):.3f}]")
...
def create_comparison_movie(results, output_path="discrete/time_smoothing_comparison.mp4", fps=10):
    ...
    # Initialize plots
    im1 = axes[0, 0].imshow(rho_unsmoothed[:, :, 0].T, origin='lower', vmin=rho_vmin, vmax=rho_vmax, cmap='viridis')
    im2 = axes[0, 1].imshow(rho_smoothed[:, :, 0].T, origin='lower', vmin=rho_vmin, vmax=rho_vmax, cmap='viridis')
    im3 = axes[1, 0].imshow(rho_vx_unsmoothed[:, :, 0].T, origin='lower', vmin=rho_vx_vmin, vmax=rho_vx_vmax, cmap='RdBu_r')
    im4 = axes[1, 1].imshow(rho_vx_smoothed[:, :, 0].T, origin='lower', vmin=rho_vx_vmin, vmax=rho_vx_vmax, cmap='RdBu_r')
...
def analyze_smoothing_effect(results):
    ...
    print(f"Smoothing factor: {rms_dt_rho_unsmoothed / rms_dt_rho_smoothed:.2f}x reduction")
    
    # Mean and std of rho fields
    print(f"Field statistics:")
    print(f"Unsmoothed - mean: {np.mean(rho_unsmoothed):.6f}, std: {np.std(rho_unsmoothed):.6f}")
    print(f"Smoothed - mean: {np.mean(rho_smoothed):.6f}, std: {np.std(rho_smoothed):.6f}")
    
    # Correlation between rho fields
    correlation_rho = np.corrcoef(rho_unsmoothed.flatten(), rho_smoothed.flatten())[0, 1]
    ...
    print(f"Smoothing factor: {rms_dt_rho_vx_unsmoothed / rms_dt_rho_vx_smoothed:.2f}x reduction")
    
    # Mean and std of rho[v_x] fields
    print(f"Field statistics:")
    print(f"Unsmoothed - mean: {np.mean(rho_vx_unsmoothed):.6f}, std: {np.std(rho_vx_unsmoothed):.6f}")
    print(f"Smoothed - mean: {np.mean(rho_vx_smoothed):.6f}, std: {np.std(rho_vx_smoothed):.6f}")
    
    # Correlation between rho[v_x] fields
    correlation_rho_vx = np.corrcoef(rho_vx_unsmoothed.flatten(), rho_vx_smoothed.flatten())[0, 1]
...
def main():
    ...
    datasets = create_test_datasets(positions, vs, deltat * subsample_factor, dims, time_sigma_values)
